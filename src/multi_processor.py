...
def extract_complete_10k_sections(text: str) -> Dict[str, str]:
    ...
    # Strategy 1: Find section headers and extract until next header
    header_based = extract_by_section_headers(soup)
    sections.update(header_based)
    
    # Strategy 2: Extract large content blocks and map to sections
    if len(sections) < 4:  # If we're missing key sections
        print("   🔄 Supplementing with content-based extraction...")
        content_based = extract_by_content_blocks(soup)
        
        # Only add if we get more content
...
def extract_content_between_elements(start_element, end_element) -> str:
    ...
    # Include the header element's text
    header_text = start_element.get_text(strip=True)
    if header_text:
        content_parts.append(header_text)
    
    # Move to next element and collect until end_element
...
def extract_by_content_blocks(soup: BeautifulSoup) -> Dict[str, str]:
    ...
    for element in soup.find_all(['div', 'p', 'table']):
        text = element.get_text(strip=True)
        if len(text) > 2000:  # Only substantial blocks
...
def categorize_content_blocks(blocks: List[str]) -> Dict[str, List[str]]:
    ...
    for block in blocks:
        ...
        # Only assign if we have good confidence (at least 2 keyword matches)
...
def is_newly_downloaded_file(filename: str) -> bool:
    """
    Check if file is in the new format (downloaded by our code)
    Format: {TICKER}_{YEAR}_10K.txt
    """
    # Check for files downloaded by our new code
    pattern = r'^(AAPL|MSFT|TSLA)_(2023|2024)_10K\.txt$'
...
def process_10k_files():
    """Process all NEWLY downloaded 10-K files with improved extraction"""
    
    raw_dir = "../data/raw"
    processed_dir = "../data/processed"
    os.makedirs(processed_dir, exist_ok=True)
    
    print("PROCESSING NEWLY DOWNLOADED 10-K FILES (IMPROVED EXTRACTION)")
    
    # List all files and filter for newly downloaded ones
    all_files = os.listdir(raw_dir)
    new_files = [f for f in all_files if is_newly_downloaded_file(f)]
    
    print(f" Found {len(all_files)} total files in raw directory")
    print(f" Processing {len(new_files)} newly downloaded files")
    
    if not new_files:
        print(" No newly downloaded files found!")
        print("\nExpected format: {TICKER}_{YEAR}_10K.txt")
        print("Example: AAPL_2023_10K.txt, MSFT_2024_10K.txt")
        return
    ...
    for filename in sorted(new_files):
        ...
        print(f"\n PROCESSING: {filename} ({company} - {year})")
        print("-" * 50)
        ...
        try:
            ...
            if file_size < 50000:
                print(f"     Small file size - may be placeholder")
            ...
            for section_name, content in sections.items():
                section_filename = f"{company}_{year}_{section_name}.txt"
            ...
            if section_count == 0:
                print("    No sections extracted - using fallback")
                # Fallback: Save entire file as a single section
                fallback_filename = f"{company}_{year}_full.txt"
                fallback_path = os.path.join(processed_dir, fallback_filename)
                with open(fallback_path, 'w', encoding='utf-8') as f:
                    f.write(raw_text[:1000000])  # First 1MB if very large
                print(f"    Saved full text (fallback): {min(file_size, 1000000):,} chars")
            else:
                print(f"    Saved {section_count} sections ({total_chars:,} total chars)")
    ...
    if processed_files:
        print(f" Created {len(processed_files)} files in processed directory:")
        for file in sorted(processed_files):
            filepath = os.path.join(processed_dir, file)
            size = os.path.getsize(filepath)
            print(f"   - {file} ({size:,} bytes)")
    else:
        print(" No files were created in processed directory")
